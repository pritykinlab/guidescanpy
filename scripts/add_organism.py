...
import gzip
from guidescanpy.flask.db import insert_chromosome_query, insert_gene_query, insert_exon_query


def insert_chromosome(organism, chr2acc_file):
    result = []
    with open(chr2acc_file,) as f:
        reader = csv.DictReader(f, delimiter='\t')
        for row in reader:
            insert_chromosome_query(accession=row['Accession.version'], name=row['#Chromosome'], organism=organism)
...
def get_attr(result, which, multiple=False, missing_ok=False):
    ...
    if which not in attrs:
        if not missing_ok:
            raise KeyError(f"Missing attribute {which}. Annotation line_no={result['line_no']}")
        else:
            return [] if multiple else None

    if not multiple:
        if len(attrs[which]) > 1:
            raise RuntimeError(f"Multiple attribute {which}. Annotation line_no={result['line_no']}")
...
if __name__ == '__main__':
    ...
    organism, gtf_gz, chr2acc = sys.argv[1:]
