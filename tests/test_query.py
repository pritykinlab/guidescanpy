...
def test_query_gene_symbol(app):
    ...
    response = app.test_client().get(
        "/query?organism=sacCer3&enzyme=cas9&query-text=CNE1"
    )
    assert response.mimetype == "application/json"
    data = json.loads(response.data)
    assert (
...
def test_query_entrez_id(app):
    response = app.test_client().get(
        "/query?organism=sacCer3&enzyme=cas9&query-text=852343"
    )
    assert response.mimetype == "application/json"
    data = json.loads(response.data)
    assert len(data) == 1
...
def test_query_chr(app):
    response = app.test_client().get(
        "/query?organism=sacCer3&enzyme=cas9&query-text=chrIX:202200-202300"
    )
    assert response.mimetype == "application/json"
    data = json.loads(response.data)
    assert len(data) == 1
...
def test_query_chr_bad(app):
    ...
    response = app.test_client().get(
        "/query?organism=sacCer3&enzyme=cas9&query-text=chrXX:1000-2000"
    )
    assert response.mimetype == "application/json"
    data = json.loads(response.data)
    assert len(data) == 1
...
def test_query_chr_pos_bad(app):
    ...
    response = app.test_client().get(
        "/query?organism=sacCer3&enzyme=cas9&query-text=chrV:182113225-182113225"
    )
    assert response.mimetype == "application/json"
    data = json.loads(response.data)
    assert len(data) == 1
