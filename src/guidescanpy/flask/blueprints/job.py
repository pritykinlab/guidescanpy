import logging
from flask import Blueprint, jsonify, abort, render_template, request, make_response
...
def job(job_id):
    ...
    if result is not None:
        ...
        first_region = ''
    return render_template('job.html', job_id=job_id, status=status, result=result, first_region=first_region)
    ...
    if format == 'json':
        ...
        for _, v in result['queries'].items():
            for hit in v['hits']:
                ...
                start = int(start) - 1  # convert from 1-indexed inclusive to 0-indexed inclusive; end remains unchanged
