...
class RegionFileParser:
    def __init__(self, lines=None, filepath=None, organism=None):
        assert any([lines, filepath]) and not all([lines, filepath]), "Specify one of lines/filepath"
        self.lines = lines
    ...
    def parse_line(self, line):
        ...
        if match is not None:
            ...
            if region := create_region_query(self.organism, line):
                return region['region_name'], region['chromosome_name'], region['start_pos'], region['end_pos']
...
class BedRegionFileParser(RegionFileParser):
    def parse_line(self, line):
        ...
        if match is not None:
            chr, start, end, region_name = match.group(1), int(match.group(2)), int(match.group(3)), match.group(4)
