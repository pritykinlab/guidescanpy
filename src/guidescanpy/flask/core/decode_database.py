...
def map_coord_to_sequence(fasta_record_dict, sgrna, chr, pos, strand):
    ...
    return (
        str(fasta_record_dict[chr].seq[pos_start:pos_end].upper()),
        pos_start,
        pos_end,
    )
...
def calc_cfd(sg, wt, pam):
    return calc_cfd_e(sg, wt, pam, mm_scores, pam_scores)
...
def decode_off_targets(sam_record, genome, delim, fasta_record_dict):
    ...
    for distance, pos in ots:
        ...
        offtarget, offtarget_pos_start, offtarget_pos_end = map_coord_to_sequence(
            fasta_record_dict, sgrna, chrm, pos, strand
        )
...
def output_succinct(record, offtargets):
    ...
    if offtargets:
        ...
        for offtarget in offtargets:
            ...
            if (
                offtarget["distance"] == 0
                and flag is False
                and offtarget["cfd"] is not None
                and cfd_sum is not None
            ):
...
def parse_args():
    ...
    p.add_argument("fasta_file", help="FASTA file for resolving off-target sequences.")
...
if __name__ == "__main__":
    ...
    def decode_ot(record):
        return decode_off_targets(record, genome, delim, fasta_dict)
